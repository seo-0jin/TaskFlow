...
import { useProjectTemplateVM } from '../../viewmodels/useProjectTemplateVM';
import { TemplatePopup } from '../../components/popup/TemplatePopup';

const ProjectTemplatePage = () => {
  const { popupVM, openCreateTemplatePopup } = useProjectTemplateVM();
  ...
  return (
    <div className={styles.project_templates}>
      <div className={styles.top_content}>
        <Button text='템플릿 추가' onClick={openCreateTemplatePopup} />
      </div>

      <div className={styles.content_area}>
        {/* 테이블 */}
        <div className={styles.table_area}>
          <TableComp columns={projectTemplateColumns} data={pagedData} rowKey={(row) => row.templateId} />
        </div>

        {/* Pagination */}
        <div className={styles.pagination_area}>
          <Pagination pageInfo={pageInfo} onChangePage={handleChangePage} onChangeSize={handleChangeSize} />
        </div>
      </div>
