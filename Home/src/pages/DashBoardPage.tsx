...
import {
  ...
  type RecentIssue,
} from '../viewmodels/useDashBoardVM';
...
const DashBoardPage = () => {
  ...
  const { dueTodayCount, inProgressCount, unassignedCount, doneLast7DaysCount } = state.summary;
  ...
  const recentIssueColumns: TableColumn<RecentIssue>[] = [
    {
      ...
      width: '40%',
      render: (row) => (
        <span className='ellipsis_data' title={row.title} style={{ cursor: 'pointer', fontWeight: 500 }}>
          {row.title}
        </span>
      ),
    },
    ...
    <section className={styles.dashboard}>
      <div className={styles.main_area}>
        ...
        <div className={styles.contents_wrap}>
          <div className={styles.issue_status_wrap}>
            <div className={styles.title_wrap}>
              ...
              <p className={styles.issue_description}>등록된 모든 프로젝트의 이슈를 요약해 보여줍니다.</p>
            </div>
            <ul>
              <li className={styles.li_item} onClick={() => onClickSummary(SummaryType.DUE_TODAY)}>
                <div>
              ...
              <li className={styles.li_item} onClick={() => onClickSummary(SummaryType.IN_PROGRESS)}>
                <div>
              ...
              <li className={styles.li_item} onClick={() => onClickSummary(SummaryType.UNASSIGNED)}>
                <div>
              ...
              <li className={styles.li_item} onClick={() => onClickSummary(SummaryType.DONE_7D)}>
                <div>
        ...
        <div className={styles.chart_wrap}>
          ...
          <div className={styles.bar_chart_wrap}>
            <div className={styles.bar_chart}>
              ...
              <HorizontalBarChart data={toIssueStatusBarData(issueStatusChart)} showValueLabel />
            </div>
            <div className={styles.bar_chart}>
              <h2 className={styles.title}>프로젝트 진행 현황</h2>
              <HorizontalBarChart data={toProjectProgressBarData(projectProgress)} showValueLabel />
            </div>
          </div>
        </div>
        ...
        <div className={styles.recent_wrap}>
          ...
          <TableComp columns={recentIssueColumns} data={recentIssues} rowKey={(row) => row.issueId} />
        </div>
      </div>
    </section>
  );
